...
from apps.marketplace.models import BaseListing


class IsTagOwner(permissions.BasePermission):
    def has_object_permission(self, request, view: APIView, listing: BaseListing):
        return listing.tag.tag_group.store.user == request.user


def check_listing_store_permissions(request, view: APIView, listing: BaseListing):
